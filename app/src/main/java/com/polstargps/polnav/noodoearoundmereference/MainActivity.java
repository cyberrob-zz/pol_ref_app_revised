...
public class MainActivity extends AppCompatActivity {
    ...
    public static String POI_DB = "poi_db";

    public static void copyPolNavDb(final Context context) {
        DB_PATH = context.getFilesDir().getAbsolutePath() + File.separator + POI_DB;
        final File polNavDbDir = new File(DB_PATH);
        if (polNavDbDir.exists()) {
            Log.d("POLSTAR", "DB already existed.");
            return;
        }
        FileUtil.copyFolder(context.getAssets(), POI_DB, DB_PATH);
        Log.d("POLSTAR", "POL db copied to " + DB_PATH);
    }
        ...
        rxPermissions
                ...
                .subscribe(new Action1<Boolean>() {
                    ...
                    public void call(Boolean granted) {
                        if (!granted) {
                            Toast.makeText(getBaseContext(), "Enable permission before using this app.", Toast.LENGTH_SHORT).show();
                        }
                    }
                });
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        queryManager = NearbyPoiQueryManager.getInstance(DB_PATH + File.separator + "poi.db");
